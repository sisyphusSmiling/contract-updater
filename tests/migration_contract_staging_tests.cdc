...
access(all) var fooUpdateHash:[UInt8] = []
...
access(all) var aUpdateHash:[UInt8] = []
...
access(all) var bUpdateHash:[UInt8] = []
...
access(all) var cUpdateHash:[UInt8] = []
...
access(all) fun testStageContractSucceeds() {
    ...
    Test.assertEqual(fooUpdateHash, evt.codeHash)
...
access(all) fun testStageMultipleContractsSucceeds() {
    ...
    Test.assertEqual(cUpdateHash, cEvt.codeHash)
    ...
    Test.assertEqual(bUpdateHash, bEvt.codeHash)
    ...
    Test.assertEqual(aUpdateHash, aEvt.codeHash)
...
access(all) fun testUnstageContractSucceeds() {
    ...
    Test.assertEqual(fooAccount.address, evt.address)
    Test.assertEqual([] as [UInt8], evt.codeHash)
    Test.assertEqual("Foo", evt.contractIdentifier)
...
access(all) fun getStagedContractCodeHash(contractAddress: Address, contractName: String): [UInt8]? {
    let stagedContractCodeResult = executeScript(
        "../scripts/migration-contract-staging/get_staged_contract_code_hash.cdc",
        [contractAddress, contractName]
    )
    Test.assert(stagedContractCodeResult.error == nil, message: "Problem retrieving result of getStagedContractCodeHash()")
    return stagedContractCodeResult.returnValue as! [UInt8]?
}
